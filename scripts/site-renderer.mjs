...
function layout({ profile, links, title, description, pathName, body, schema = [], extraScript = '' }) {
  ...
  <link rel="icon" href="/favicon.svg" type="image/svg+xml"><script src="/shared/theme.js"></script><link rel="stylesheet" href="/shared/redesign.css?v=__BUILD_VERSION__">
...
function blacklight(profile, links, products, gallery) {
  ...
  <form class="quote-form" data-blacklight-form data-phone="${esc(links.blacklightWhatsApp)}"><label>Nome<input name="nome" required autocomplete="name"></label><label>Tipo de peça<select name="tipo" required autocomplete="off"><option value="">Selecione</option>${products.map((p) => `<option>${esc(p.name)}</option>`).join('')}<option>Projeto personalizado</option></select></label><label class="wide">Descrição<textarea name="descricao" required rows="4" autocomplete="off"></textarea></label><label>Quantidade<input name="quantidade" type="number" min="1" inputmode="numeric" autocomplete="off" required></label><label>Medidas aproximadas<input name="medidas" autocomplete="off" placeholder="Ex.: 12 × 8 × 3 cm"></label><label>Cor<input name="cor" autocomplete="off"></label><label>Finalidade<input name="finalidade" autocomplete="off" required></label><label>Prazo desejado<input name="prazo" autocomplete="off" placeholder="Data ou período"></label><fieldset class="wide"><legend>Possui arquivo 3D?</legend><label><input type="radio" name="arquivo" value="Sim" required> Sim</label><label><input type="radio" name="arquivo" value="Não"> Não</label></fieldset><label class="wide">Observações<textarea name="observacoes" rows="3" autocomplete="off"></textarea></label><button class="button primary wide" type="submit">Abrir mensagem no WhatsApp</button><p class="form-status wide" role="status" aria-live="polite"></p></form></div></section>`
